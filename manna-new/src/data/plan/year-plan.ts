...
export interface DayReading {
  ...
  proverb: string;      // e.g. "Proverbs 1:1-7"
}

// Chapter counts for every book, in canonical order.
...
const OT_CHAPTERS = flatten(OT_BOOKS);   // 929 chapters (excl. Psalms & Proverbs)
const NT_CHAPTERS = flatten(NT_BOOKS);   // 260 chapters

/** Group an array into `days` roughly-equal consecutive chunks, rendered as ranges. */
function chunkToRanges(chapters: string[], days: number): string[] {
  ...
  for (let d = 0; d < days; d++) {
    ...
    // Same book? render "Genesis 1-3". Different books? "Genesis 50 - Exodus 2".
...
/** Psalms: 150 psalms over 365 days — cycle through, some repeat as reflection. */
function psalmForDay(day: number): string {
  const n = ((day - 1) % 150) + 1;
  return `Psalm ${n}`;
}

/** Proverbs: 31 chapters — read the chapter matching the day of month. */
function proverbForDay(day: number): string {
  const n = ((day - 1) % 31) + 1;
  return `Proverbs ${n}`;
}
